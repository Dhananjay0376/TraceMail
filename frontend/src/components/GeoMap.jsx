              ...
              <Marker key={i} position={[h.lat, h.lon]}>
                <Popup>
                  <div style={{ color: '#0f172a', fontSize: '0.85rem' }}>
                    <strong>{h.is_origin ? '📍 Earliest Originating Node' : `Hop #${h.hop_index || i + 1}`}</strong><br />
                    <b>IP:</b> {h.ip}<br />
                    <b>Location:</b> {h.city || 'Unknown'}, {h.country || ''}<br />
                    <b>ISP/ASN:</b> {h.org || 'Unknown'}
            ...
            {trace.map((h, idx) => (
              <tr key={idx} style={{ borderBottom: '1px solid #1e293b' }}>
                <td style={{ padding: '8px 6px' }}>
                  <span className={h.is_origin ? 'badge badge-red' : 'badge badge-blue'}>
                    {h.is_origin ? 'ORIGIN' : `#${h.hop_index || idx + 1}`}
                  </span>
                </td>
                <td style={{ padding: '8px 6px' }}><code>{h.ip}</code></td>
                <td style={{ padding: '8px 6px' }}>{h.city ? `${h.city}, ${h.country || ''}` : h.country || 'Unknown'}</td>
